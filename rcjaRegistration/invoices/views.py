...
@login_required
def summary(request):
    invoices = Invoice.invoicesForUser(request.user).prefetch_related(
        "invoicepayment_set",
        "school",
        "campus",
        "invoiceToUser",
    ...
    context = {
        "invoices": invoices,
        "showCampusColumn": Campus.objects.filter(
            school__schooladministrator__user=request.user
        ).exists(),
    }
...
def mentorInvoicePermissions(request, invoice):
    return (
        request.user.schooladministrator_set.filter(school=invoice.school).exists()
        or invoice.invoiceToUser == request.user
    )
...
def updateInvoicedDate(invoice, mentor):
    if mentor and (
        invoice.invoicedDate is None
        or invoice.invoicedDate > datetime.datetime.today().date()
    ):
...
def setInvoiceTo(request, invoiceID):
    if request.method == "POST":
        ...
        return JsonResponse(
            {
                "id": invoiceID,
                "invoiceTo": request.user.fullname_or_email(),
                "success": True,
            }
        )
    else:
...
def setCampusInvoice(request, invoiceID):
    if request.method == "POST":
        ...
        if not invoice.campusInvoicingAvailable():
            return HttpResponseForbidden(
                "Campus invoicing is not available for this event"
            )
        ...
        for campus in Campus.objects.filter(
            school=invoice.school, baseeventattendance__event=invoice.event
        ).distinct():
...
def editInvoicePOAJAX(request, invoiceID):
    if request.method == "POST":
        ...
        return JsonResponse(
            {"id": invoiceID, "number": request.POST["PONumber"], "success": True}
        )
...
def sendOverdueEmail(invoice, event, user):
    ...
    if event.paymentDueDate < datetime.datetime.today().date() - datetime.timedelta(
        42, 0, 0, 0, 0, 0, 0
    ):
        # Beyond Overdue
    ...
    elif event.paymentDueDate < datetime.datetime.today().date() - datetime.timedelta(
        21, 0, 0, 0, 0, 0, 0
    ):
        # Well Overdue
        text_content = render_to_string(
        ...
        if form.is_valid():
            for eventID in form.cleaned_data["events"]:
                ...
                for invoice in invoices:
                    if (
                        invoice.amountDueInclGST_unrounded() > 0.05
                        and event.paymentDueDate < datetime.datetime.today().date()
                    ):
