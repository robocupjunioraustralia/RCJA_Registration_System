...
def commonSetUp(self):
    ...
    self.schoolAdmin2 = SchoolAdministrator.objects.create(
        school=self.school2, user=self.user2
    )
    self.school3 = School.objects.create(
        name="School 3", state=self.state1, region=self.region1
    )
    self.schoolAdmin3 = SchoolAdministrator.objects.create(
        school=self.school3, user=self.user3
    )
    ...
    self.eventDue43Days = Event.objects.create(
        ...
        endDate=(datetime.datetime.now() + datetime.timedelta(days=5)).date(),
        registrationsOpenDate=(
            datetime.datetime.now() + datetime.timedelta(days=-43)
        ).date(),
        registrationsCloseDate=(
            datetime.datetime.now() + datetime.timedelta(days=-43)
        ).date(),
...
class TestOverdueEmails(TestCase):
    ...
    def setUp(self):
        ...
        createStudents(self)
        self.invoice0 = Invoice.objects.get(
            event=self.eventDueToday, school=self.school1
        )
        self.invoice1 = Invoice.objects.get(
            event=self.eventDueYesterday, school=self.school1
        )
        self.invoice21 = Invoice.objects.get(
            event=self.eventDue21Days, school=self.school1
        )
        self.invoice22 = Invoice.objects.get(
            event=self.eventDue22Days, school=self.school1
        )
        self.invoice42 = Invoice.objects.get(
            event=self.eventDue42Days, school=self.school1
        )
        self.invoice43 = Invoice.objects.get(
            event=self.eventDue43Days, school=self.school1
        )
    ...
    def testLoginRequired(self):
        ...
        self.assertEqual(
            response.url, f"/accounts/login/?next=/invoices/overdueEmails/"
        )
    ...
    def testMemberAccess(self):
        ...
        self.assertContains(
            response, "You do not have permission to view this page", status_code=403
        )
    ...
    def testEventsGiven(self):
        ...
        self.assertNotIn(
            (self.eventDueToday.pk, self.eventDueToday.name), returned_events
        )
