...
def dashboard(request):
    ...
    if request.user.currentlySelectedSchool:
        usersEventAttendances = BaseEventAttendance.objects.filter(
            school=request.user.currentlySelectedSchool
        )
    else:
        usersEventAttendances = BaseEventAttendance.objects.filter(
            mentorUser=request.user, school=None
        )
    ...
    futureEvents = (
        Event.objects.filter(
            Q(registrationsOpenDate__gt=datetime.datetime.today())
            | Q(registrationsOpenDate__isnull=True),
    ...
    if request.method == "GET" and not "viewAll" in request.GET:
        openForRegistrationEvents = openForRegistrationEvents.filter(
            Q(state=currentState) | Q(globalEvent=True) | Q(state__typeGlobal=True)
        )
    ...
    openForRegistrationCompetitions = openForRegistrationEvents.filter(
        eventType="competition"
    )
    openForRegistrationWorkshops = openForRegistrationEvents.filter(
        eventType="workshop"
    )
    ...
    outstandingInvoices = sum(
        [1 for invoice in invoices if invoice.amountDueInclGST() > 0.05]
    )  # Rounded because consistent with what user sees and not used in subsequent calculations
    ...
    if showAssociationPrompt:
        ...
        request.user.save(
            update_fields=["associationPromptShown"], skipPrePostSave=True
        )
...
def getDivisionsMaxReachedWarnings(event, user):
    ...
    for availableDivision in event.availabledivision_set.prefetch_related(
        "division"
    ).all():
        if availableDivision.maxDivisionRegistrationsForSchoolReached(user):
            divisionsMaxReachedWarnings.append(
                f"{availableDivision.division}: Max {event.registrationName()}s for school for this event division reached. Contact the organiser if you want to register more {event.registrationName()}s in this division."
            )

        if availableDivision.maxDivisionRegistrationsTotalReached():
            divisionsMaxReachedWarnings.append(
                f"{availableDivision.division}: Max {event.registrationName()}s for this event division reached. Contact the organiser if you want to register more {event.registrationName()}s in this division."
            )
...
def getAvailableToCopyTeams(request, event):
    ...
    copiedTeamsList = (
        Team.objects.filter(**filterDict)
        .filter(copiedFrom__isnull=False)
        .values_list("copiedFrom", flat=True)
    )
    ...
    availableToCopyTeams = teams.exclude(
        pk__in=copiedTeamsList
    )  # Exclude already copied teams
...
def details(request, eventID):
    ...
    if event.boolWorkshop():
        ...
        totalRegistrations = event.baseeventattendance_set.exclude(
            team__withdrawn=True
        ).count()

    context = {
        "event": event,
        "availableDivisions": event.availabledivision_set.prefetch_related("division"),
        "divisionPricing": event.availabledivision_set.exclude(
            division_billingType="event"
        ).exists(),
        "teams": teams,
        "workshopAttendees": workshopAttendees,
        "showCampusColumn": BaseEventAttendance.objects.filter(**filterDict)
        .exclude(campus=None)
        .exists(),
        "billingTypeLabel": billingTypeLabel,
        "hasAdminPermissions": coordinatorEventDetailsPermissions(request, event),
        "maxEventRegistrationsForSchoolReached": event.maxEventRegistrationsForSchoolReached(
            request.user
        ),
...
def cms(request, eventID):
    ...
    if event.cmsEventId:
        return redirect(
            settings.CMS_EVENT_URL_VIEW.replace("{EVENT_ID}", event.cmsEventId)
        )
    ...
    cmsPayload = {
        ...
        + datetime.timedelta(minutes=settings.CMS_JWT_EXPIRY_MINUTES),
    }
...
class CreateEditBaseEventAttendance(LoginRequiredMixin, View):
    def common(self, request, event, eventAttendance):
        ...
        if eventAttendance and not mentorEventAttendanceAccessPermissions(
    ...
    def delete(
        self, request, teamID=None, attendeeID=None, eventID=None, sourceTeamID=None
    ):
...
def getEventsForSummary(state, year):
    ...
    eventList = Event.objects.filter(state=state, year=year).order_by(
        "startDate", "endDate"
    )
    ...
    for event in eventList:
        ...
        if event.startDate == event.endDate:
            ...
            eventDict["date"] = (
                f"{event.startDate.strftime('%d/%m/%Y')} - {event.endDate.strftime('%d/%m/%Y')}"
            )
        ...
        if event.eventType == "competition":
            ...
            eventDict["participants_three"] = (
                f"{fPercent}%F, {mPercent}%M, {oPercent}% other"
            )
        else:  # Workshop
            # Initialise counting variables
            studentNumber = 0
